...
4. Under [**Environment Variable**](http://kagenti-ui.localtest.me:8080/Import_New_Agent#select-environment-variable-sets), select:
   ...
   - Under `Name` put `MCP_URL` and under `Value` put `http://image-tool:8000/mcp`
4. In the **Agent Source Repository URL** field, fill in `https://github.com/SiennaChien/agent-examples-fork/tree/image_agent/a2a/image_service`
5. For **Git Branch or Tag**, use the default `image_agent` branch (or select another as needed).
...
1. Under **Tool Source Repository URL**, use `https://github.com/SiennaChien/agent-examples-fork/tree/image_tool/mcp/image_tool`
1. Fill in `image_tool` for the branch section
